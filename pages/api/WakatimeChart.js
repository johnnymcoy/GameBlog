...
import { BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid, ResponsiveContainer, Line, ComposedChart, Area } from 'recharts';
...
const DateSelection = ({setDisplayRange}) => {
    ...
    return(
        <div className='m-0 mr-4 flex justify-end flex-row '>
            <button  
                className={buttonClass}
                onClick={() => {onButtonClick(7)}}>7 Days</button>
            <button 
                className={buttonClass}
                onClick={() => {onButtonClick(14)}}>14 Days</button>
            <button 
                className={buttonClass}
                onClick={() => {onButtonClick(30)}}>30 Days</button>
            <button 
                className={buttonClass}
                onClick={() => {onButtonClick(90)}}>90 Days</button>
            <button 
                className={buttonClass}
                onClick={() => {onButtonClick(0)}}>Max</button>
        </div>
    )
}
    ...
    useEffect(() => {
        ...
        (err, response) => {
            ...
            const transformData = (data, lastNumDays) => {
                ...
                const reversedData = recentData.sort((a, b) => new Date(a.range.date) - new Date(b.range.date));
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        <div className="custom-tooltip p-2 border-2 border-gray-200 border-opacity-60 dark:border-gray-700 bg-white dark:bg-black" >
...
<ResponsiveContainer width="100%" aspect={2.5} className="h-auto">
    ...
    <div className="flex justify-end text-base mr-8 space-y-0 pb-6 pt-0 md:space-y-0">
